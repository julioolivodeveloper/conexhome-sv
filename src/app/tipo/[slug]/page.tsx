...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const type = PROPERTY_TYPES.find((t) => t.slug === slug)
  if (!type) return { title: 'Tipo no encontrado' }

  return {
    title: `${type.label}s en venta y alquiler en El Salvador`,
    description: `Encuentra ${type.label.toLowerCase()}s en venta y alquiler en El Salvador. Publicaciones gratuitas en ConexHome SV.`,
    openGraph: {
      title: `${type.label}s en El Salvador | ConexHome SV`,
      description: `Explora ${type.label.toLowerCase()}s disponibles en El Salvador.`,
    },
  }
}
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="bg-navy py-8 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <p className="text-slate-400 text-sm mb-1">
            ...
            {' › '}
            <span className="text-white">{type.label}s</span>
          </p>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">
            {type.icon} {type.label}s en El Salvador
          </h1>
          <p className="text-slate-300 text-sm mt-1">
            {count ?? 0} {type.label.toLowerCase()}s disponibles
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex gap-3 mb-6">
          <Link
            href={`/comprar?tipo=${encodeURIComponent(type.value)}`}
            className="px-4 py-2 bg-accent text-white text-sm font-medium rounded-xl hover:bg-accent-dark transition-colors"
          >
            {type.label}s en venta
          </Link>
          <Link
            href={`/alquilar?tipo=${encodeURIComponent(type.value)}`}
        ...
        {properties.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-lg font-semibold text-slate-600 mb-2">
              Aún no hay {type.label.toLowerCase()}s publicadas
            </h2>
        ...
        {/* Other types */}
        <div className="mt-12">
          <h2 className="text-sm font-semibold text-slate-600 mb-3">Otros tipos de propiedad</h2>
          <div className="flex flex-wrap gap-2">
            {PROPERTY_TYPES.filter((t) => t.slug !== slug).map((t) => (
              <Link
                key={t.slug}
                href={`/tipo/${t.slug}`}
